...
Decision recorded in `docs/adr/0017-open-mcp-catalog-deployment-secrets.md`. Remote MCP servers are added via a deploy-time Integration Catalog and Flue `useMcpConnection`, authenticated with deployment Worker secrets resolved at Coworker render. OAuth authorization flows and refresh-token storage are deferred, so OAuth-only MCP servers are unsupported. Catalog rows use Bearer auth and mount all discovered tools by default; a reviewed row may instead use Basic auth and a fixed tool allowlist. Blast radius for those calls is the intersection of mounted tools and the secret’s scopes. Flue does not re-execute an unresolved ordinary MCP call after runtime interruption. An uncertain MCP timeout or disconnect is treated as potentially unknown by Coworker instructions: do not intentionally repeat an equivalent effectful call; surface the ambiguity to the human. This is reliability guidance, not a generic harness retry gate. D6/D11 still forbid putting reusable MCP/CLI tokens in the sandbox. D13/D14 continue to govern **native** Credential Proxy ops (GitHub branch/PR/checkpoint). Authenticated `wrangler`/`gh` in Daytona remain rejected. Optional `cfRead` typed passthrough is no longer the only Cloudflare path once a Cloudflare MCP row is catalogued; prefer minting a read-scoped `CLOUDFLARE_API_TOKEN` for pilots.

### Agent observability pilot (2026-09-20)

Flue's native `invoke_agent`, `chat`, and `execute_tool` spans flow through Cloudflare Workers Traces at 100% sampling. The single-user pilot captures full prompt, response, tool-argument, and tool-result content and exports the same traces to the existing Honeycomb destination and Langfuse Cloud US while retaining Cloudflare persistence. Coworker also mounts an optional Langfuse MCP connection with only observation, metrics, score, and health read tools. It uses those tools only when the Slack request explicitly asks to analyze or improve a completed prior run, treats trace content as untrusted evidence, and follows the normal tested-PR workflow for any resulting code change. It never automatically reviews, merges, or deploys its own work.

### Model-authored Block Kit replies (2026-09-23)

Decision recorded in `docs/adr/0019-model-authored-block-kit-replies.md`. Coworker uses Markdown for ordinary replies and may author native Slack Block Kit for charts, tables, composed layouts, or controls backed by implemented handlers. `reply_in_slack_thread` retains one bound destination and a required text fallback. Without blocks it posts `markdown_text`; with blocks it posts validated `blocks` plus top-level `text`, never `markdown_text` in the same call. The fallback includes the substantive takeaway and a verbal chart summary for notifications and screen readers. Owner code validates the complete Slack payload and rejects unsupported types, nested elements, and limit violations with repairable errors rather than silently dropping data. The initial capability set is display-only: markdown, header, divider, text-only section/context, data visualization, and `data_table` with only `raw_text` or `raw_number` cells. Slack-managed table sorting and pagination require no app callback. Image blocks, image elements, external media fields, and app-callback elements without registered handlers are rejected. Both reply shapes disable link and media unfurls; ordinary links remain clickable. App-callback controls require implemented, signature-verified, authorized handlers with defined state and retry behavior before their action IDs are allowed. The run card remains owner-rendered. Native Slack charts need no browser snapshot, `files:write` scope, or sandbox resource profile. Browser-based UI verification and arbitrary diagrams are separate future decisions.
